...
%>%
  ...
  mutate(var=factor(var, levels = c("S", "I", "R", "Factual", "Removed")))
...
plot_1 <- ggplot(fit, aes(x=date))+
  geom_line(data=subset(fit, !(var %in% c("Factual", "Removed"))), aes(y=values, col=var))+
...
plot_2 <- ggplot(fit, aes(x=date))+
  ...
  geom_point(data=subset(fit, var=="Removed"), aes(y=values), na.rm = TRUE, col="green", shape=3)+
  scale_color_manual(values=c("blue", "red", "green"), name="Atvejas")+
  scale_x_date(breaks="1 week")+ 
