...
ggplot(df,aes(x=valstybe, y=value, fill=valstybe))+
        geom_bar(stat='identity') +
        geom_text(aes(label=value, y=value), size=2.5, vjust=-0.5)+
        labs(x="Valstybė", y="Mirčių skaičius", title="Mirčių skaičius top 10 pasaulio šalių", subtitle="Šaltinis: JHCSSE, skaičiavimai: corona-stat.lt" )+
        theme(axis.text=element_text(size=8),
              axis.text.x=element_text(angle=45, hjust=1))
```
###Top 10 pasaulio šalių pagal susirgimų skaičių mirtingumo rodiklis
```{r,echo=FALSE}
df <- read.csv("./data/data_world.csv",
               stringsAsFactors = FALSE)%>%
