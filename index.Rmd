...
max.date <- max(data_world_map$date)
```
...
ggplot(df, aes(fill=value))+
  ...
  scale_fill_gradientn(colours=brewer.pal(name ="YlOrRd", n=9),
                       ...
                       breaks=my_breaks,
                       labels=my_breaks)+
  labs(title=paste0("Patvirtintų apsikrėtusiųjų skaičiaus pokytis per parą (duomenys ", max.date,")"),
       subtitle = "Šaltinis: JHCSSE, skaičiavimai: Corona-Stat.lt")
...
###Top 10 pasaulio šalių pagal mirčių skaičių
...
Žemiau lentelėje pateikiamas santykis tarp atliktų testų bei stebimų asmenų. Duomenų šaltinis SAM. Deje neturime pilnos informacijos visoms dienoms, neaišku, kiek iš altiktų testų yra altikti pakartotini testai, taip pat ebėjonių kelia paskutines dienas nedidėjęs stebimų asmenų (kurie potencialiai turėjo kontaktą su koronoviruso nešiotojais) skaičius. Todėl tikėtina, jog ypatingai paskutinių dienų ištyrimų statistika yra klaidinga (per duag optimistinė).
...
data_lt %>%
  ...
  mutate(tested_perc = color_bar("lightgrey")(tested_perc))%>%
  rename(Data=date,
