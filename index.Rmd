...
Turint omenyje visuos šiuos ribotumus, žemiau pristatome SIR kalibruoto modelio Lietuvoje prognozes. Modelis rekalbibruojasi kasdien, gavus daugiau duomenų apie užsikrėtimų skaičių. Nepaisnt to, griežtai rekomenduojama **nežiūrėti į šį modelį kaip realią prognozę, o tik aip į galimą scenarijų**.

```{r}
df <- data_lt_country_cum %>%
        ungroup()%>%
        filter(var %in% c("active", "recovered"))%>%
        spread(var, values)%>%
        filter(.$active>=10,
               date!=max(date))
...
Kairysis grafikas iliustruoja faktinius sakčius. Labai gerai matomas augimo eksponentiškumas. Dešinysis grafikas iliustruoja logaritmuotus skaičius, tad matoma, kaip beveik labai "gražiai" logaritmuoti įverčiai sugula ant tiesės. Tiesa laikotarpio pabaigoje pradedamas apstebėti nukrypimas, kuris gali suponuoti, jog koronaviruso užsikrėtimų skaičiaus nebeauga taip sparčiai ir yra pasiektas "pikas".

```{r}
plot_1 <- 
ggplot(df, aes(date, active))+
...
plot_2 <- 
ggplot(df, aes(date, log2(active)))+
  geom_point(col="red", size=3)+
...
grid.arrange(plot_1, plot_2, nrow=1, top="Patvirtinti COVID-19 atvejai Lietuvoje \n Šaltinis ir skaičiavimai: Corona-Stat.lt")
```
...
fit<- fit %>%
  mutate(time=seq(min(df$date), by="day", length.out=nrow(fit)))%>%
  rename(date=time)%>%
  left_join(., df, by="date")%>%
  rename(Factual=active,
         Recovered=recovered)%>%
  gather(var, values, -date)%>%
  mutate(var=factor(var, levels = c("S", "I", "R", "Factual", "Recovered")))
...
```{r}
plot_1 <- ggplot(fit, aes(x=date))+
  geom_line(data=subset(fit, !(var %in% c("Factual", "Recovered"))), aes(y=values, col=var))+
  scale_color_manual(values=c("blue", "red", "green"), name="Atvejas")+
  scale_x_date(breaks="1 week")+ 
...
plot_2 <- ggplot(fit, aes(x=date))+
  geom_line(data=subset(fit, !(var %in% c("Factual", "Recovered"))), aes(y=values, col=var))+
  geom_point(data=subset(fit, var=="Factual"), aes(y=values), na.rm = TRUE, col="red", shape=4)+
  geom_point(data=subset(fit, var=="Recovered"), aes(y=values), na.rm = TRUE, col="green", shape=3)+
  scale_color_manual(values=c("blue", "red", "green"), name="Atvejas")+
  scale_x_date(breaks="1 week")+ 
  scale_y_continuous(trans = log2_trans(),
                     minor_breaks = trans_breaks("log2", function(x) 2^(x/2)),
...
grid.arrange(plot_1, plot_2, nrow=1, top="Koronaviruso susirgimo prognozė remiantis SIR modeliu \n Šaltinis ir skaičiavimai: Corona-Stat.lt")
```
