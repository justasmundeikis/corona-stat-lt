...
data_world_map <- read.csv("./data/data_world_map.csv",
               header = TRUE,
               stringsAsFactors = FALSE)%>%
...
ggplot(df, aes(x=date, y=value, fill=var))+
        ...
        scale_fill_manual(values=c("red", "green", "black"),
                          labels=c("Aktyvūs", "Pasveikę", "Mirę") )+
        scale_x_date(breaks="1 week")+
        labs(title=paste0("Europos ekonominės erdves koronaviruso atvejų skaičius ir pasiskirstymas (naujausi duomenys ", max.date, ")"),
             subtitle = "Šaltinis: JHCSSE, skaičiavimai: Corona-Stat.lt",
             x="Data",
             y="Skaičius")+
        theme(legend.title=element_blank(),
...
ggplot(df,aes(x=reorder(valstybe,value), y=value)) +
  geom_bar(stat='identity',
           fill="steelblue")+
  geom_text(aes(label=value, y=value), size=3, vjust=-0.75)+
  geom_hline(yintercept = 100, col="red", size=1.1)+
...
ggplot(df, aes(x=date, y=diff)) +
        ...
        labs(title=paste0('Covid-19 susirgimų pokytis TOP5 ir Baltijos valstybėse įvedus karantiną',
                          max.date), subtitle=" Šaltinis: corona-stat.lt", x="Data", y="Susirgimų skaičius")+
  ...
  filter(valstybe%in%c("Airija", "Austrija", "Belgija", "Bulgarija", "Čekija", "Danija", 
        ...
        ggplot(df, aes(x=date, y=value, fill=var)) +
                ...
                labs(title=paste0('Covid-19 atvejai TOP6 Europos šalyse ',
                                  max.date," duomenimis"), subtitle=" Šaltinis: corona-stat.lt", x="Data", y="Susirgimų skaičius") +
