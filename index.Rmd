...
plot_2 <- 
ggplot(df, aes(date, log2(values)))+
  geom_point(col="red", size=3)+
...
Opt <- optim(c(0.5, 0.5), RSS, method = "L-BFGS-B", lower = c(0, 0), upper = c(10, 10))
...
fit<- fit %>%
  ...
  rename(Factual=values)%>%
  gather(var, values, -date)%>%
  mutate(var=factor(var, levels = c("S", "I", "R", "Factual")))
...
plot_1 <- ggplot(fit, aes(x=date))+
  geom_line(data=subset(fit, var!="Factual"), aes(y=values, col=var))+
...
plot_2 <- ggplot(fit, aes(x=date))+
  geom_line(data=subset(fit, var!="Factual"), aes(y=values, col=var))+
  geom_point(data=subset(fit, var=="Factual"), aes(y=values), na.rm = TRUE)+
  scale_color_manual(values=c("blue", "red", "green"), name="Atvejas")+
  scale_x_date(breaks="1 week")+ 
